...
class MediaPostController extends Controller
{
    ...
    {
        if ($request->string('type')->value() === 'press') {
            return redirect()->route('admin.pages.media.news.edit');
        }
        if ($request->string('type')->value() === 'blog') {
            return redirect()->route('admin.pages.media.stories.edit');
        }
    ...
    public function newsCreate(): Response { return Inertia::render('Admin/Pages/MediaNewsForm', ['post' => null]); }
    public function blogCreate(): Response { return Inertia::render('Admin/Pages/MediaNewsForm', ['post' => null, 'kind' => 'blog']); }

    public function newsEdit(MediaPost $mediaPost): Response
    {
        $mediaPost->load(['featuredImage', 'openGraphImage']);
        $translations = $mediaPost->translations ?? [];
        $translations['en'] = array_replace(['title' => $mediaPost->title, 'excerpt' => $mediaPost->excerpt ?? '', 'content' => $mediaPost->content ?? ''], $translations['en'] ?? []);
        $translations['ar'] = $translations['ar'] ?? [];
        return Inertia::render('Admin/Pages/MediaNewsForm', ['post' => ['id' => $mediaPost->id, 'title' => $mediaPost->title, 'slug' => $mediaPost->slug, 'excerpt' => $mediaPost->excerpt, 'content' => $mediaPost->content, 'event_date' => $mediaPost->event_date?->format('Y-m-d'), 'published_at' => $mediaPost->published_at?->format('Y-m-d\\TH:i'), 'is_featured' => $mediaPost->is_featured, 'is_published' => $mediaPost->is_published, 'sort_order' => $mediaPost->sort_order, 'seo_title' => $mediaPost->seo_title, 'seo_description' => $mediaPost->seo_description, 'featured_image' => WebsiteContent::assetUrl($mediaPost->featuredImage), 'open_graph_image' => WebsiteContent::assetUrl($mediaPost->openGraphImage), 'translations' => $translations], 'kind' => request()->routeIs('admin.pages.media.stories.post.edit') ? 'blog' : 'news']);
    }

    public function blogEdit(MediaPost $mediaPost): Response
    {
        return $this->newsEdit($mediaPost);
    }
    ...
    private function sanitizeTranslations(array $translations): array
    {
        ...
        foreach (['en', 'ar'] as $locale) {
            ...
            foreach (['excerpt', 'content'] as $key) {
