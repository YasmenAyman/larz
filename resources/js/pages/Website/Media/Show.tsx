            ...
            <article className="bg-night text-ink">
                <header className="relative overflow-hidden border-b border-hairline/30 bg-night px-6 pt-40 pb-20 sm:px-10 lg:px-24">
                    <div className="mx-auto max-w-[1000px]">
                        <a href="/media" className="inline-flex items-center gap-2 text-[0.62rem] tracking-[0.2em] text-ink-muted uppercase hover:text-ink"><ArrowLeft className="size-3.5 rtl:rotate-180" /> {t('Back to media')}</a>
                        <p className="mt-12 text-[0.6rem] tracking-[0.22em] text-gold uppercase">{post.category ? t(post.category) : null}{post.category && post.date ? ' · ' : ''}{post.date}</p>
                ...
                <section className="bg-paper px-6 py-20 text-paper-ink sm:px-10 sm:py-24 lg:px-0">
                    <div className="mx-auto grid max-w-[1000px] gap-12 md:grid-cols-[1.2fr_0.8fr] md:items-start">
                        <div className="text-sm leading-[1.9] text-paper-muted">
                            <p>{post.excerpt}</p>
                            {post.content && <div className="mt-6 space-y-4 text-sm leading-[1.9] [&_p]:text-paper-muted" dangerouslySetInnerHTML={{ __html: post.content }} />}
