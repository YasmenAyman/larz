...
const translations: Record<string, string> = {
    ...
    'General CV Submissions': 'طلبات السير الذاتية العامة',
    'General CV submissions': 'طلبات السير الذاتية العامة',
    'job Applications': 'طلبات التوظيف',
    'internship Applications': 'طلبات التدريب',
    Applicant: 'المتقدم',
    'Job/program': 'الوظيفة/البرنامج',
    Status: 'الحالة',
    Assigned: 'المسؤول',
    Notes: 'ملاحظات',
    CV: 'السيرة الذاتية',
...
function useTr() {
    ...
    const tr = (v: string) => (locale === 'ar' ? translations[v] ?? v : v);
    return { tr, ar: locale === 'ar' };
}
...
function ApplicationRow({ item, users, base }: { item: ApplicationItem; users: StaffUser[]; base: string }) {
    ...
    return (
        <tr className="border-b border-slate-800/70 align-middle">
            <td className="px-4 py-4 align-middle"><p>{item.name}</p><p className="text-xs text-slate-500">{item.email}</p><p className="mt-1 text-xs text-slate-600">{item.submission_date ?? ''}</p></td>
            <td className="px-4 py-4 align-middle text-center text-slate-400">{item.job ?? 'General CV'}</td>
            <td className="px-4 py-4 align-middle">
                <div className="flex flex-col items-center gap-2">
                    ...
                    <select value={data.status} onChange={(event) => setData('status', event.target.value)} className="rounded border border-slate-700 bg-slate-900 px-2 py-1 text-xs">
                        <option value="">{tr('Select status')}</option>
                        {statuses.map((status) => <option key={status} value={status}>{status}</option>)}
                    </select>
                </div>
            </td>
            <td className="px-4 py-4 align-middle text-center">
                <select value={data.assigned_to} onChange={(event) => setData('assigned_to', event.target.value)} className="rounded border border-slate-700 bg-slate-900 px-2 py-1 text-xs">
                    <option value="">{tr('Unassigned')}</option>
                    {users.map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}
                </select>
            </td>
            <td className="px-4 py-4 align-middle text-center">
                <textarea rows={2} value={data.admin_notes} onChange={(event) => setData('admin_notes', event.target.value)} className="mx-auto block w-48 rounded border border-slate-700 bg-slate-900 px-2 py-1 text-xs" placeholder={tr('Internal notes')} />
            </td>
            <td className="px-4 py-4 align-middle text-center">
                {item.has_resume ? <a href={`/admin/${base}/${item.id}/cv`} className="text-emerald-400">{tr('Download')}</a> : tr('None')}
            </td>
            <td className="px-4 py-4 align-middle text-center">
                <button type="button" disabled={processing} onClick={save} className="rounded bg-emerald-600 px-3 py-2 text-xs">{tr('Save')}</button>
            </td>
        </tr>
    );
}
