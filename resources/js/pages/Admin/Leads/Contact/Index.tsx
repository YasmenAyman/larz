...
export default function Index({ inquiries, filters }: { inquiries: { data: Inquiry[]; current_page: number; last_page: number }; filters: { search?: string; status?: string } }) {
    ...
    return (
        <AdminLayout>
            <Head title={tr('Contact Inquiries')} />
            <div className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
                <div><Breadcrumbs items={['Dashboard', 'Leads', 'Contact inquiries'].map(tr)} /><h1 className="mt-3 text-3xl font-semibold">{tr('Contact inquiries')}</h1></div>
                <Notification message={flash?.success} />
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <SearchInput value={filters.search ?? ''} onChange={(value) => router.get('/admin/contact-inquiries', { search: value }, { preserveState: true })} placeholder={tr('Search')} />
                ...
                {inquiries.data.length === 0
                    ? <EmptyState title={tr('No inquiries yet')} message={tr('Submissions from the public contact form will appear here.')} />
