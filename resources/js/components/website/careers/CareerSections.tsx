...
function CareerFormModal({ title, roleId, internshipId, jobSlug, onClose }: { title: string; roleId?: number; internshipId?: number; jobSlug?: string; onClose: () => void }) {
    ...
    const { t } = useI18n();
    const { data, setData, post, processing, errors, recentlySuccessful, setError, clearErrors } = useForm<CareerFormData>({ job_position_id: roleId ? String(roleId) : '', internship_program_id: internshipId ? String(internshipId) : '', name: '', email: '', phone: '', resume: null, cover_note: '', cover_letter: '', consent_at: new Date().toISOString(), source_url: typeof window !== 'undefined' ? window.location.href : '', _hp_website: '' });
    ...
    const submit = (event: React.FormEvent) => {
        ...
        post(action, { forceFormData: true, preserveScroll: true, onSuccess: () => onClose() });
    };
