...
class PhotoGalleryController extends Controller
{
    ...
    public function store(StorePhotoGalleryItemRequest $request, MediaUploadService $uploads): RedirectResponse { $newAsset = null; try { DB::transaction(function () use ($request, $uploads, &$newAsset) { $data = $request->safe()->except('image'); $newAsset = $uploads->storePublicImage($request->file('image'), 'media/gallery'); $data['media_asset_id'] = $newAsset->id; $data['is_published'] = $request->boolean('is_published'); $data['is_active'] = $request->boolean('is_active'); PhotoGalleryItem::create($data); }); } catch (\Throwable $exception) { if ($newAsset) $uploads->delete($newAsset); throw $exception; } $this->flushCaches(); return back()->with('success', 'Photo added.'); }
    public function update(UpdatePhotoGalleryItemRequest $request, PhotoGalleryItem $photoGalleryItem, MediaUploadService $uploads): RedirectResponse { $oldAsset = $photoGalleryItem->media; $newAsset = null; try { DB::transaction(function () use ($request, $uploads, $photoGalleryItem, &$newAsset) { $data = $request->safe()->except('image'); if ($request->hasFile('image')) { $newAsset = $uploads->storePublicImage($request->file('image'), 'media/gallery'); $data['media_asset_id'] = $newAsset->id; } $data['is_published'] = $request->boolean('is_published'); $data['is_active'] = $request->boolean('is_active'); $photoGalleryItem->update($data); }); } catch (\Throwable $exception) { if ($newAsset) $uploads->delete($newAsset); throw $exception; } if ($request->hasFile('image') && $oldAsset) $uploads->deleteIfUnreferenced($oldAsset); $this->flushCaches(); return back()->with('success', 'Photo updated.'); }
    public function destroy(PhotoGalleryItem $photoGalleryItem, MediaUploadService $uploads): RedirectResponse { $asset = $photoGalleryItem->media; $photoGalleryItem->delete(); if ($asset) $uploads->deleteIfUnreferenced($asset); $this->flushCaches(); return back()->with('success', 'Photo moved to trash.'); }
