                ...
                <form onSubmit={submit} className="space-y-5 rounded-2xl border border-white/10 bg-[#161619]/90 p-6 shadow-xl">
                    <div className="grid gap-5 sm:grid-cols-2">
                        <FormField label="Title" error={errors.title}>
                            <input required value={data.title} onChange={(event) => setData('title', event.target.value)} className={inputClass} placeholder="Award name or milestone" />
                        </FormField>
                    ...
                    <FormField label="Description" error={errors.description}>
                        <textarea rows={4} value={data.description ?? ''} onChange={(event) => setData('description', event.target.value)} className={inputClass} placeholder="A short line on what this recognition was for." />
                    </FormField>
                    <div className="grid gap-5 sm:grid-cols-2">
