        ...
        >
            <div className="mx-auto max-w-[1440px] px-4 sm:px-8">
                    ...
                    <div className="flex gap-6">
                        {testimonials.map((item, i) => (
                            <figure
                                ...
                                className="flex basis-full shrink-0 gap-6 flex-col md:flex-row rounded-2xl bg-[#121212] p-6 md:basis-[calc(50%-12px)]"
                            >
                                {item.image ? <img src={item.image} alt={item.name} className={`h-[300px] w-full shrink-0 rounded-xl object-cover grayscale sm:w-[50%]`} draggable={false} /> : <div className={`h-[300px] w-full shrink-0 rounded-xl bg-white/5 sm:w-[50%]`} aria-hidden="true" />}
                                <div className={`flex min-w-[100px] flex-col justify-between ${isRtl ? 'text-right' : ''}`}>
                ...
                <div className={`mt-8 flex items-center gap-3 ${isRtl ? 'justify-start' : 'justify-end'}`}>
                    <button type="button" aria-label="Previous testimonial" onClick={scrollPrev} className={`grid size-10 place-items-center bg-white/20 text-white transition-colors hover:bg-neutral-800 ${isRtl ? 'rounded-r-2xl' : 'rounded-l-2xl'}`}>
                        <PrevIcon className="size-4" strokeWidth={1.5} />
                    </button>
                    <button type="button" aria-label="Next testimonial" onClick={scrollNext} className={`grid size-10 place-items-center bg-white/20 text-white transition-colors hover:bg-neutral-800 ${isRtl ? 'rounded-l-2xl' : 'rounded-r-2xl'}`}>
