    ...
    return (
        <section className="relative overflow-hidden border-b border-hairline/30 bg-night" style={{ backgroundImage: `radial-gradient(ellipse 60% 70% at 82% 20%, rgba(164, 121, 43, 0.30) 0%, rgba(115, 79, 27, 0.2) 35%, transparent 72%), linear-gradient(110deg, rgba(6, 4, 4, 0.98) 28%, rgba(2, 2, 4, 0.73) 100%)`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat' }}>
            <div className="relative mx-auto flex min-h-[700px] max-w-[1440px] flex-col justify-center px-6 pt-36 pb-16 sm:px-10 lg:px-24">
                <Eyebrow className="text-ink">{hero.eyebrow ?? 'Media'}</Eyebrow>
                ...
                <a href="#newsletter" className="mt-8 inline-flex w-fit items-center gap-3 border border-gold px-5 py-3 text-[0.62rem] tracking-[0.2em] text-ink uppercase hover:bg-gold/10">Subscribe <ArrowRight className="size-3.5" strokeWidth={1.5} /></a>
            </div>
        </section>
    );
}
    ...
    return (
        <section className="bg-[#EFEFF1] py-20 text-paper-ink sm:py-24">
            <div className="mx-auto max-w-[1440px] px-6 sm:px-10 lg:px-24">
                <Eyebrow tone="light">{settings.eyebrow ?? 'News & press releases'}</Eyebrow>
                <h2 className="mt-5 text-4xl font-light sm:text-[2.5rem]">{settings.heading ?? "What's happening at LARZ."}</h2>
                    ...
                    {posts.filter((post) => post.type === 'press').map((post) => (
                        <article key={post.slug} className="border border-paper-muted/25 bg-[#f7f7f8]">
                            ...
                            <div className="p-4">
                                ...
                                <a href={`/media/${post.slug}`} className="mt-4 inline-flex items-center gap-1 text-[0.8rem] tracking-[0.18em] text-paper-muted uppercase">Read more <ArrowRight className="size-3" strokeWidth={1.5} /></a>
                            </div>
                        </article>
                    ))}
                </div>
...
export function MediaStories({ posts, settings }: { posts: MediaPost[]; settings: SectionSettings }) {
    return (
        <section className="bg-night py-20 sm:py-24">
            <div className="mx-auto max-w-[1440px] px-6 sm:px-10 lg:px-24">
                <Eyebrow className="text-ink">{settings.eyebrow ?? 'Blogs'}</Eyebrow>
                <h2 className="mt-5 text-4xl font-light text-ink sm:text-[2.5rem]">{settings.heading ?? 'Stories & insight.'}</h2>
                    ...
                    {posts.filter((post) => post.type === 'blog').map((post) => (
                        <article key={post.slug} className="border border-hairline/50 bg-surface-deep">
                            ...
                            <div className="p-4">
                                ...
                                <a href={`/media/${post.slug}`} className="mt-4 inline-flex items-center gap-1 text-[0.9rem] tracking-[0.18em] text-ink-muted uppercase">Read article <ArrowRight className="size-3" strokeWidth={1.5} /></a>
                            </div>
                        </article>
                    ))}
                </div>
...
export function MediaGallery({ gallery, settings }: { gallery: string[]; settings: SectionSettings }) {
    ...
    return (
        <section className="bg-[#EFEFF1] py-20 text-paper-ink sm:py-24">
            <div className="mx-auto max-w-[1440px] px-6 sm:px-10 lg:px-24">
                <Eyebrow tone="light">{settings.eyebrow ?? 'Photo gallery'}</Eyebrow>
                <h2 className="mt-5 text-4xl font-light sm:text-[2.5rem]">{settings.heading ?? 'Inside our communities.'}</h2>
                {settings.description && <p className="mt-4 max-w-2xl text-sm text-paper-muted">{settings.description}</p>}
    ...
    return (
        <section id="newsletter" className="relative overflow-hidden bg-night py-20 text-center sm:py-32" style={{ backgroundImage: 'radial-gradient(ellipse 100% 100% at 55% 10%, rgba(164, 121, 43, 0.30) 0%, rgba(115, 79, 27, 0.2) 55%, transparent 72%)', backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat' }}>
            <div className="relative mx-auto max-w-2xl px-6">
                <Eyebrow className="justify-center text-ink">{settings.eyebrow ?? 'Newsletter'}</Eyebrow>
                <h2 className="mt-5 text-3xl font-light text-ink sm:text-[3.5rem]">{settings.heading ?? 'Never miss an update.'}</h2>
                ...
                <form className="mx-auto mt-8 flex max-w-[600px] flex-col gap-2 sm:flex-row" onSubmit={submit} noValidate>
                    ...
                    <button type="submit" disabled={processing} className="inline-flex items-center justify-center gap-3 border border-gold px-6 py-3 text-[0.8rem] tracking-[0.2em] text-ink uppercase hover:bg-gold/10 disabled:opacity-60">{processing ? 'Subscribing...' : 'Subscribe'} <ArrowRight className="size-3" strokeWidth={1.5} /></button>
                </form>
            </div>
        </section>
    );
